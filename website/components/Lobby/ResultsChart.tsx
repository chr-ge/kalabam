...
const calculateMostAnswers = (data) =>
  data.reduce((acc, { answers }) => {
...
export const ResultsChart: FC<ResultsChartProps> = ({
  correct,
  answers,
  answersCount,
}) => {
  ...
  return (
    <Flex h={height} maxW='2xl' mx='auto' mb='16' justify='space-between'>
      {data
        .filter((_, i) => i < answersCount)
        .map(({ color, answers }, i) => (
          <Flex key={color} direction='column' justify='end' align='center'>
            <Flex align='center' justify='center'>
              {correct.includes(i) && (
                <Icon as={ImCheckmark} color={color} mr='2' boxSize={6} />
              )}
              <Text color={color} align='center' fontSize='5xl'>
                {answers}
              </Text>
            </Flex>
            <Box
